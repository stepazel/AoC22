...
next_grid = grid.copy()
next_removed_rolls_count = 999
total_removed_rolls_count = 0
while next_removed_rolls_count > 0:
    ...
    for y in range(len(grid)):
        for x in range(len(grid[0])):
            ...
            if adjacent_rolls_count < 4: # remove
                accessible_rolls_count += 1
                next_grid[y][x] = '.'

    total_removed_rolls_count += accessible_rolls_count
    next_removed_rolls_count = accessible_rolls_count # reset the count
    grid = next_grid.copy()


print(total_removed_rolls_count)
