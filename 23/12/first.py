...
def check_options(springs: str, pattern: list) -> int:
    if not pattern:
        if "#" not in springs:
            return 1
        else:
...
while True:
    ...
    springs_str = line.split(' ')[0]
    pattern_array = [int(s) for s in line.split(' ')[1][:-1].split(',')]
    number_of_combinations = check_options(springs_str, pattern_array)
