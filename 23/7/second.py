...
def get_card_type_value(card: str) -> int:
    counts = get_translates_card_counts(card)
    match counts:
        case [5]:
...
def get_translates_card_counts(card: str) -> list:
    counts = get_counts_of_labels(card)
    if 'J' in card:
        count_of_j = card.count('J')
        if count_of_j == 5:
            return counts

        if count_of_j == counts[0]:
            counts[1] += count_of_j
        else:
            counts[0] += count_of_j

        counts.remove(count_of_j)
    return counts
